...
class AutoCompleteTextField<T> extends StatefulWidget {
  ...
  final bool submitOnSuggestionTap, clearOnSubmit, clearOnTap;
...
class AutoCompleteTextFieldState<T> extends State<AutoCompleteTextField> {
  ...
  int minLength;
  bool submitOnSuggestionTap, clearOnSubmit, clearOnTap;
  ...
  List<T> getSuggestions(List<T> suggestions, Comparator<T> sorter,
    ...
    suggestionsR.sort(sorter);
    if (suggestionsR.length > maxAmount) {
      suggestionsR = suggestionsR.sublist(0, maxAmount);
    }
    print(suggestionsR);
    return suggestionsR;
  }
...
class SimpleAutoCompleteTextField extends AutoCompleteTextField<String> {
  ...
  State<StatefulWidget> createState() => new AutoCompleteTextFieldState<String>(
        ...
        String comprare = removeDiacritics(a);
        String comprareB = removeDiacritics(b);

        return comprare.compareTo(comprareB);
      }, (item, query) {
        if (query == "") {
          return true;
        }
        String comprare = removeDiacritics(item.toLowerCase());
        String comprareB = removeDiacritics(query.toLowerCase());
        return comprare.contains(comprareB);
      },
          suggestionsAmount,
          submitOnSuggestionTap,
          clearOnSubmit,
